 ...
 *  Le contenu est à largeur fixe : il ne se recompose pas, il est dévoilé.
 *  L'<aside> ne fait que réserver la gouttière de 72px dans le flux — c'est
 *  le panneau interne, en absolu, qui s'élargit et survole l'app. Sinon
 *  l'élargissement volerait 268px au contenu et redimensionnerait tout. */
        ...
        <div className="fs-inner">
          ...
          <form className="fs-add" onSubmit={submitAdd}>
            <div className="fs-add-field">
              ...
              />
              {name.trim() && (
                <button type="submit" className="icon-btn ok fs-reveal" disabled={pending} data-tip="Envoyer" aria-label="Envoyer" style={{ width: 26, height: 26 }}>
              ...
              <>
                ...
                {props.incoming.map((r) => (
                  <div key={r.id} className="fs-row fs-req">
                    ...
                    <button className="icon-btn ok fs-reveal" data-tip="Accepter" aria-label="Accepter" onClick={() => props.onAccept(r.id)}>
                      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
                        <path d="M20 6 9 17l-5-5" />
                      </svg>
                    </button>
                    <button className="icon-btn neutral fs-reveal" data-tip="Refuser" aria-label="Refuser" onClick={() => props.onDecline(r.id)}>
                ...
                {online.map((f) => (
                  <div key={f.id} className="fs-row clickable">
                    <div
                      ...
                      data-tip="Voir le profil" aria-label="Voir le profil"
                    >
                    ...
                    <button className="icon-btn neutral fs-reveal" data-tip="Retirer" aria-label="Retirer" onClick={() => props.onRemove(f.id)}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6">
                        <path d="M18 6 6 18M6 6l12 12" />
                      </svg>
                    </button>
